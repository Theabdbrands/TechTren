...
const MobileChatSidebar = ({
    ...
    // Hooks - IDENTICAL to ChatSidebar.tsx
    ...
    // Prefetch conversations on mount - IDENTICAL to ChatSidebar.tsx
    useEffect(() => {
    ...
    const filteredConversation = conversationsData?.data?.filter((conversation: Conversation) => conversation.conversationId.startsWith('chat_')) || [];
    ...
    const handleSelectChat = (conversation: Conversation) => {
        ...
        // Parse and set messages from the selected conversation - IDENTICAL to ChatSidebar.tsx
    ...
    const handleStartEdit = (e: React.MouseEvent, conversation: Conversation) => {
        ...
        // Try to extract title from the first user message - IDENTICAL to ChatSidebar.tsx
        try {
            const parsed = JSON.parse(conversation.contentJson);
    ...
    const handleSaveEdit = async (e: React.MouseEvent, conversation: Conversation) => {
        ...
        // Update the title in the contentJson - IDENTICAL to ChatSidebar.tsx
        try {
            const parsed = JSON.parse(conversation.contentJson);
    ...
    // Helper to extract title from conversation - IDENTICAL to ChatSidebar.tsx
    ...
    // Helper to get message count - IDENTICAL to ChatSidebar.tsx
    ...
    // Group chats by date - IDENTICAL to ChatSidebar.tsx
                ...
                <div className="space-y-1">
                    {chats.map((chat) => (
                        ...
                        >
                            ...
                            {editingChatId !== chat.conversationId && (
                                <div className="flex items-center opacity-100 transition-all duration-300 flex-col">
                                    <button
    ...
    // IDENTICAL to ChatSidebar.tsx
    // const conversations = conversationsData?.data || [];
    // const { today, yesterday, previous7Days, previous30Days, older } = groupChatsByDate(conversations);
    const { today, yesterday, previous7Days, previous30Days, older } = groupChatsByDate(filteredConversation);
    ...
    return (
        <>
            ...
            >
                ...
                {/* Logo */}
                <img src={Logo} alt="Logo" className="w-8 mr-10" />
            ...
            <AnimatePresence>
                {sidebarOpen && (
                    <>
                        ...
                        // className="fixed inset-0 glass backdrop-blur-sm z-[90]"
                        // style={{ background: "rgba(20, 20, 20, 0.30)" }}
                        />
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 left-0 h-full glass w-full z-[100] flex flex-col !rounded-none"
                            style={{
                        ...
                        >
                            ...
                            <div className="border-b border-white/10 pb-4 px-4 flex items-center flex-col gap-5">
                                {/* Close Button */}
                                <button
                                    className="absolute top-4 glass right-6 text-white p-2 hover:bg-white/10 rounded-lg transition-colors"
                                    onClick={() => setSidebarOpen(false)}
                                    style={{ background: "rgba(20, 20, 20, 0.30)" }}>
                                    <X size={20} />
                                </button>
                            ...
                            {/* Chat History - IDENTICAL structure to ChatSidebar.tsx */}
                            ...
                            {/* Model Selector - IDENTICAL to ChatSidebar.tsx */}
                            <div ref={dropdownRef} className="relative flex flex-col items-center mt-8 px-4 pb-4">
                                <div className="text-white text-sm mb-2 font-semibold">Selected Model</div>
                                <button
                                    onClick={() => setIsOpen(!isOpen)}
                                    className="flex items-center justify-between w-full px-4 py-2 text-white rounded-lg shadow-md transition-all duration-300 cursor-pointer"
                                    style={{
                                ...
                                {isOpen && (
                                    <div
                                        className="absolute bottom-full mb-2 w-full glass"
                                        style={{
                            ...
                            <div className="absolute inset-0 pointer-events-none -z-10">
                                <img src={noise} alt="noise" className="w-full h-full object-cover opacity-50" />
                                <div className="purple-shadow bottom-10 left-20 w-1/2 h-[150px]"></div>
